...
class FuturesOneShot:
    ...
    # ========================================================
    # LIMIT BUY (POST-ONLY / GTX) WITH RETRY (3x, Jeda 1s, Opsi B Dinamis)
    # ========================================================
    async def open_position(self, close_price):
        ...
        for attempt in range(1, max_retries + 1):
            if attempt > 1:
                logging.info(f"[ENTRY] Menunggu jeda 1 detik sebelum percobaan retry ke-{attempt}...")
                await asyncio.sleep(1)
                try:
                    ticker = await self.request("GET", "/fapi/v1/ticker/price", {"symbol": self.symbol})
                    current_price = Decimal(ticker["price"])
                    logging.info(f"[ENTRY] Recalculate harga running terbaru: {current_price}")
                except Exception as e:
                    logging.warning(f"[ENTRY] Gagal mengambil ticker terbaru, memakai acuan sebelumnya ({current_price}): {e}")

            # Floor Grid ke tick terdekat di bawah/sama dengan current_price
            floor_price = (current_price // self.price_tick) * self.price_tick
            ...
            if quantity < self.min_qty: 
                raise RuntimeError(f"Quantity {quantity} di bawah minimum {self.min_qty}")
            ...
            logging.info(f"[ENTRY] BUY LIMIT (POST-ONLY / GTX) - Percobaan {attempt}/{max_retries}")
            logging.info(f"[ENTRY] Reference Price : {current_price}")
            logging.info(f"[ENTRY] Target Price    : {formatted_price} (Grid floor di bawah reference)")
            ...
            try:
                ...
                if attempt < max_retries and e.code == -5022:
                    logging.info(f"[ENTRY] Terkena penolakan Post-Only -5022. Bersiap mencoba ulang ({attempt}/{max_retries})...")
                    continue

                await self.notifier.send("⚠️ ORDER ENTRY DITOLAK", f"Order Post-Only {self.symbol} ditolak setelah {attempt}x percobaan: {e.msg}", "WARNING")
                return False
