...
class FuturesOneShot:
    ...
    async def place_take_profit(self):
        ...
        formatted_qty = f"{self.entry_qty:.{self.qty_precision}f}"

        logging.info(f"[TP] Target NET PnL = +{self.target_net_pnl}")
        logging.info(f"[TP] TP price (Post-Only) = {formatted_tp_price}")
...
def parse_args():
    ...
    parser.add_argument("--interval", type=str, default="30m", help="Interval candle (contoh: 1m, 5m, 15m, 30m, 1h)")
